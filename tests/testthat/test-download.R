...
test_that("download_nomis_data handles all parameters", {
  skip_if_no_api()
  skip_on_cran()
  
  # Mock to avoid actual API calls
  mock_resp <- tibble::tibble(
    GEOGRAPHY_CODE = "K02000001",
    OBS_VALUE = 100
  )
  
  local_mocked_bindings(
    make_api_request = function(...) mock_resp
  )
  
  result <- download_nomis_data(
    id = "NM_1_1",
    time = "latest",
    geography = "TYPE499:K02000001"
  )
  
  expect_s3_class(result, "tbl_df")
})

test_that("download_nomis_data handles caching", {
  skip_on_cran()
  ...
  result1 <- download_nomis_data("NM_1_1", time = "latest")
  expect_equal(call_count, 1)
  
  # Second call - should use cache
  result2 <- download_nomis_data("NM_1_1", time = "latest")
  expect_equal(call_count, 1) # Still 1, used cache
})

test_that("download_nomis_data builds correct query params", {
  skip_on_cran()
  
  params_captured <- NULL
  
  local_mocked_bindings(
    make_api_request = function(id, params, ...) {
  ...
  expect_equal(params_captured$time, "latest")
  expect_equal(params_captured$geography, "TYPE499")
  expect_equal(params_captured$measures, "20100")
})